...
trading_days_per_year = st.sidebar.number_input("一年交易天數 (Annualization)", min_value=1, max_value=366, value=252)
periods_per_day = st.sidebar.number_input("每日期間數 (例：逐時=24 或實際交易小時數)", min_value=1, max_value=240, value=24)
risk_free_rate = st.sidebar.number_input("無風險利率 (年化, 小數)", min_value=0.0, max_value=0.2, value=0.0, step=0.001)
...
def compute_drawdowns(equity: pd.Series) -> dict:
    ...
    for i, (e, p) in enumerate(zip(equity.values, peaks.values)):
        if e < p:  # 在回撤中
            ...
            if in_drawdown:
                # 結束事件
                depth = (trough_val - equity[start_idx]) / peaks[start_idx]  # 不直接用此定義
                # 正確深度應為 (trough - previous peak) / previous peak
    ...
    if events:
        ...
        recovery_len = max(0, max_event["end"] - max_event["trough"])  # 期間長度（含端點差）
    else:
        recovery_len = 0
...
def compute_sharpe(returns: pd.Series, ann_factor: float, rf_annual: float = 0.0) -> float:
    ...
    if len(r) < 2:
        return 0.0
    # 將年化無風險利率換算到單期
    rf_per_period = rf_annual / max(ann_factor**2, 1)  # 近似：periods_per_year = ann_factor^2
...
def compute_neil(sharpe: float, pf: float, max_dd: float, avg_dd: float, max_losing_streak: int, max_recov_len: int, total_len: int) -> float:
    # 恢復時間比例（0~1）
...
if uploaded is not None:
    try:
        ...
        sheet_name = st.selectbox("選擇要分析的工作表 (預設嘗試 'Hourly Period Analysis')", options=sheets, index=min(sheets.index("Hourly Period Analysis") if "Hourly Period Analysis" in sheets else 0, len(sheets)-1))
        ...
        col_timestamp = st.selectbox("時間欄位（可選，用於圖表 x 軸）", options=col_options, index=0)
        col_profit = st.selectbox("PROFIT 欄位（可選）", options=col_options, index=(df.columns.get_indexer(["PROFIT'])[0] + 1) if "PROFIT" in df.columns else 0)
        col_loss = st.selectbox("LOSS 欄位（可選）", options=col_options, index=(df.columns.get_indexer(["LOSS'])[0] + 1) if "LOSS" in df.columns else 0)
        col_net = st.selectbox("NetPnL/盈虧 欄位（可選，若提供則優先使用）", options=col_options, index=(df.columns.get_indexer(["NetPnL'])[0] + 1) if "NetPnL" in df.columns else 0)
        col_equity = st.selectbox("Equity/淨值 欄位（可選，若未提供則用初始資金累加 NetPnL 建立）", options=col_options, index=(df.columns.get_indexer(["Equity'])[0] + 1) if "Equity" in df.columns else 0)

        init_capital = st.number_input("初始資金（若未提供 Equity，將以此 + 累計盈虧建立）", min_value=0.0, value=1_000_000.0, step=10_000.0, format="%.2f")
        ...
        if col_net != "<無>" and col_net in df.columns:
            ...
            prof = pd.to_numeric(df[col_profit], errors="coerce").fillna(0.0) if (col_profit != "<無>" and col_profit in df.columns) else pd.Series(np.zeros(len(df)))
            loss = pd.to_numeric(df[col_loss], errors="coerce").fillna(0.0) if (col_loss != "<無>" and col_loss in df.columns) else pd.Series(np.zeros(len(df)))
            pnl = prof - loss
        ...
        neil = compute_neil(sharpe, pf, max_dd, avg_dd, max_losing, max_recov_len, total_len)
        ...
        st.caption(f"最大連續虧損次數：{max_losing}｜最大回撤恢復時間長度：{max_recov_len}（佔比 { (max_recov_len/total_len*100 if total_len>0 else 0):.2f}% ）")
        ...
        summary = pd.DataFrame({
            ...
            "RecoveryLen_Ratio(%)": [ (max_recov_len/total_len*100 if total_len>0 else 0.0) ],
            "Neil": [neil]
        })
        ...
        with pd.ExcelWriter(out, engine="xlsxwriter") as writer:
            ...
            pd.DataFrame(dd_stats["dd_series"], columns=["Drawdown"]).to_excel(writer, sheet_name="Drawdown")
            summary.to_excel(writer, sheet_name="Summary", index=False)
        st.download_button("下載分析結果 Excel", data=out.getvalue(), file_name="neil_indicator_analysis.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
